...
int main(int argc, char* argv[]) {
	...
	int *lane_array = (int *)malloc(bin_lines * sizeof(int));
	while (fgets(inbuf, sizeof(inbuf), bin_file) != NULL) { // Assuming binned file has three columns (lane, sequence, hits) delimited by \t and newlined by \n
		item = strtok(inbuf, "\t");
		lane_array[bin_file_count] = atoi(item);
		item = strtok(NULL, "\t");
	...
	for (i=0;i<lib_file_count;i++) { // Assume lib_file is smallest
		...
		for (j=0;j<bin_file_count;j++) {
			...
			if (distance<=distance_threshold) {
				sprintf(tmpbuf, "%i\t%s\t%i\t%s\t%s\t%i", lane_array[j], bin_file_seq_array[j], bin_file_hits_array[j], lib_file_seq_array[i], lib_file_srid_array[i], distance);
